    ...
    <Section bg="espresso" padding="lg" className="bg-noise relative overflow-hidden">
      ...
      <div className="relative grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {academy.courses.map((course) => (
          <div
            key={course.slug}
            className="group rounded-sm border border-sand/10 bg-white/5 p-6 transition-all duration-500 hover:border-gold/30 hover:bg-white/10"
          >
            <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-full border border-gold/20 text-gold-light transition-all duration-300 group-hover:border-gold/40 group-hover:text-gold-light group-hover:shadow-lg group-hover:shadow-gold/10">
              {iconMap[course.icon]}
            </div>
